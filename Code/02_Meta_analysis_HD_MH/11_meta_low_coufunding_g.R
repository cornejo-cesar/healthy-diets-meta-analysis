...
depr   <- subset(data, !is.na(depr_z) & unique_statistic == 1 & confounding == "Low")
anx    <- subset(data, !is.na(anx_z) & unique_statistic == 1 & confounding == "Low")
stress <- subset(data, !is.na(stress_z) & unique_statistic == 1 & confounding == "Low")
...
forest(anxiety_res,
       ...
       pch=20,
       psize=1.5)
text(-4.0, anxiety_res$k + 2, "Number of Estimates", cex=0.80, font=2)  
...
forest(stress_res,
       ...
       pch=20,
       psize=2)
text(-5.0, stress_res$k + 2, "Number of Estimates", cex=0.80, font=2)  
text(-2.5, stress_res$k + 2, "n", cex=0.80, font=2) 
text( 0.8, stress_res$k + 2, "Weight", cex=0.80, font=2) 

par(xpd=NA)
text(-10.92, stress_res$k - 23.2, paste0("Total sample population: ", total_n), cex=0.80, adj=0)
text(-10.92, stress_res$k - 24.4, paste0("Min SMD: ", round(min_smd$yi, 2),
                                        " [", round(min_smd$ci.lb, 2), ", ", round(min_smd$ci.ub, 2), "]"), cex=0.80, adj=0)
text(-10.92, stress_res$k - 25.6, paste0("Max SMD: ", round(max_smd$yi, 2),
                                        " [", round(max_smd$ci.lb, 2), ", ", round(max_smd$ci.ub, 2), "]"), cex=0.80, adj=0)                                        
text(
  x      = c(-7.2, 7.2),
  y      = -3.5,
