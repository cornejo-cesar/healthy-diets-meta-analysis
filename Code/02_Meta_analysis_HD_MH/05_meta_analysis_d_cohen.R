...
depr_meta <- data.frame(
  ...
  d                    = depr$depr_d                                           # Correlation coefficient
)                                                                               # Calculate Fisher's z and its standard error                                           # Fisher's z transformation
depr_meta$se <- sqrt((4 / depr_meta$n) + (depr_meta$d^2 / (2 * (depr_meta$n - 2))))  
...
anx_meta <- data.frame(
  ...
  d                    = anx$anx_d                                             # Correlation coefficient
)                                                                               # Calculate Fisher's z and its standard error
anx_meta$se <- sqrt((4 / anx_meta$n) + (anx_meta$d^2 / (2 * (anx_meta$n - 2))))  
...
stress_meta <- data.frame(
  ...
  d                    = stress$stress_d                                        # Correlation coefficient  
)                                                                               # Calculate Fisher's z and its standard error
stress_meta$se <- sqrt((4 / stress_meta$n) + (stress_meta$d^2 / (2 * (stress_meta$n - 2)))) 
stress_meta$vi <- stress_meta$se^2


depr_re   <- escalc(measure="SMD", yi=depr_meta$d, vi=depr_meta$vi, slab=depr_meta$slab, data=depr_meta)
anx_re    <- escalc(measure="SMD", yi=anx_meta$d, vi=anx_meta$vi, slab=anx_meta$slab, data=anx_meta)
stress_re <- escalc(measure="SMD", yi=stress_meta$d, vi=stress_meta$vi, slab=stress_meta$slab, data=stress_meta)
...
depr_resultrobust   <- robust(res_depr, cluster = pop_cohort_dataset_id, clubSandwich = TRUE, digits = 3)
...
anx_resultrobust    <- robust(res_anx, cluster = pop_cohort_dataset_id, clubSandwich = TRUE, digits = 3)
...
stress_resultrobust <- robust(res_stress, cluster = pop_cohort_dataset_id, clubSandwich = TRUE, digits = 3)
