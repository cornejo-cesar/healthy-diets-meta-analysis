...
risk_of_bias <- function(data, outcome_name, include_overall = TRUE) {
  ...
  filtered_data <- switch(outcome_name,
                          ...
                          "Longitudinal" = data %>%
                            filter(study_design == "Longitudinal") %>%
                            distinct(effect_size_id, study_design, .keep_all = TRUE),
                          stop("Invalid outcome name. Use 'Depression', 'Anxiety', 'Stress', or 'Longitudinal'.")
  )
...
rob_summary_quips_custom <- function(data,
  ...
  max_domain_column <- 8  # Set the maximum number of domain columns
